...
Color = [   
            (255, 255, 255),
            # orange
            (255, 120, 0), # darkorange - piglet
            
            # blue
            (0, 0, 139), # darkblue - lying
            (0, 0, 255), # blue - sitting
            (30, 144, 255), # dodgerblue - standing
...
def draw_boxes_with_label_and_scores(img_array, boxes, labels, scores, method, is_csl=False, in_graph=True):
    ...
    for box, a_label, a_score in zip(boxes, labels, scores):
        ...
        if a_label != NOT_DRAW_BOXES:
            num_of_objs += 1
            draw_a_rectangel_in_img(draw_obj, box, color=Color[a_label], width=3, method=method)
