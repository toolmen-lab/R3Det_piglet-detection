...
def train():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        with tf.Session(config=tfconfig) as sess:
            ...
            for step in range(cfgs.MAX_ITERATION // num_gpu+1):
                ...
                if (step > 0 and step % (cfgs.SAVE_WEIGHTS_INTE // num_gpu) == 0) or (step >= cfgs.MAX_ITERATION // num_gpu):
                    ...
                    save_ckpt = os.path.join(save_dir, '{}_'.format(cfgs.DATASET_NAME) +
                                             str((global_stepnp-1)*num_gpu) + '_model.ckpt')
